...
ico_path = os.path.join(sys._MEIPASS, 'logo.png') if hasattr(sys, '_MEIPASS') else 'logo.png'
...
def get_logo_path():
    ...
    elif os.path.exists("/usr/share/icons/hicolor/48x48/apps/logo.png"):
        return "/usr/share/icons/hicolor/48x48/apps/logo.png"
    
    # Kullanıcı ev dizinindeki bir yol
    home_dir = os.path.expanduser("~")
    user_logo_path = os.path.join(home_dir, ".enco", "logo.png")
    if os.path.exists(user_logo_path):
        return user_logo_path

    # Varsayılan olarak bulunduğu dizindeki logo.png
    return "logo.png"
...
class FileEncryptionApp(QMainWindow):
    def __init__(self):
        ...
        self.setWindowIcon(QIcon("logo.png"))  # Uygulama simgesini ekliyoruz
    ...
    def initUI(self):
        ...
        pixmap = QPixmap("logo.png")  # Logoyu yüklüyoruz (logo.png dosyasını eklediğinizden emin olun)
    ...
    def show_about_dialog(self, about_text):
        ...
        about_dialog.setWindowIcon(QIcon("logo.png"))  # Hakkında penceresi simgesi ekleniyor
